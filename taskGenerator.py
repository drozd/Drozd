...
class TaskGenerator:
	...
	def __init__(self, corpus_folder, subject_file, tail_file):
		# Create pymorphy and word2vec objects, loading names and tails
		#random.seed(1)
		self.__morph = pymorphy2.MorphAnalyzer()
		print("Word2Vec is being built")
		self.__model = vec.Word2Vec(self.__sentences(corpus_folder), size=100, window=10, min_count=5, workers=4)
		print("Word2Vec has been built")
		self.__names = []
		self.__verbs = []
		self.__tails = dict()
		cases = ['nomn', 'gent', 'datv', 'accs', 'ablt', 'loct']
		
		with open(subject_file, 'r', encoding='utf-8') as subjects:
			for name in subjects:
				self.__names.append(name.strip())
		
		verb = ''
		with open(tail_file, 'r', encoding='utf-8') as tails:
			for tail in tails:
				if not verb:
					verb = tail.strip()
